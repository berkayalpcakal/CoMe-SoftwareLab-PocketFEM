...
public class fragment_CreateGeometry extends android.support.v4.app.Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState){
        ...
        Button btnApproveGeometries = fragmentCreateGeometryView.findViewById(R.id.btnApproveGeometries);
        btnApproveGeometries.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        if (v.getId()==R.id.btnApproveGeometries)
        {
            approveGeometries();
        }
        ...
        glSurfaceView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(firstPointDone == false){
                    posFirstPoint = renderer.IntersectionTest( renderer.getNormalizedPointPosition(v, event ) );
                    firstPointDone = true;
                }
                else if(firstPointDone == true && secondPointDone == false){
                    posSecondPoint = renderer.IntersectionTest( renderer.getNormalizedPointPosition(v, event) );
...
//                             Log.d(TAG, "Ready to get third point...");
                    posThirdPoint = renderer.IntersectionTest( renderer.getNormalizedPointPosition(v, event) );
...
//                             Log.d(TAG, "Now we have three points. Ready to create Triangle!!!");
    ...
    // This function finalize geometry definition and passes data to FEMPackage
    @SuppressLint("ClickableViewAccessibility")
    public boolean approveGeometries()
    {
        saveGeometryToJSONFile(Rectangles, Triangles, "rawGeometriesToFEMPackage.json");
        LogToConsole("---------------------------------------------------------------------");
        LogToConsole("Geometry is defined to be used by FEM Package");
        LogToConsole("---------------------------------------------------------------------");
    ...
    public boolean selectEntity()
    {
        glSurfaceView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                Point = renderer.getNormalizedPointPosition(v, event );
    ...
    // This function opens PopUp-Window and asks for width and height of the rectangle
